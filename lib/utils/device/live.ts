		...
		for (const serviceName of _.keys(this.composition.services)) {
			...
			if (service.build != null) {
				...
				const serviceDirs = this.deployOpts.multiDockerignore
					? await getServiceDirsFromComposition(this.deployOpts.source, this.composition)
					: {};
				const { dockerignoreFiles } = await filterFilesWithDockerignore(this.deployOpts.source, serviceDirs);
				...
				for (const file of dockerignoreFiles) {
					try {
						...
						this.logger.logError(`Error reading .dockerignore file "${file.filePath}":\n${err}`);
						throw err;
					}
